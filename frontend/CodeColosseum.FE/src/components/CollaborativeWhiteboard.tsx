...
export function CollaborativeWhiteboard({ participants }: CollaborativeWhiteboardProps) {
  ...
  const [remoteCursors, setRemoteCursors] = useState<Array<{ userId: string; username: string; x: number; y: number; color: string }>>([]);
  ...
  useEffect(() => {
    ...
    if (currentElement) {
      ...
      if (currentElement.type === 'line' && currentElement.points) {
        ...
        ctx.stroke();
      } else if (currentElement.type === 'rectangle' && currentElement.start && currentElement.end) {
        ctx.strokeRect(
      ...
      } else if (currentElement.type === 'circle' && currentElement.start && currentElement.end) {
        const radius = Math.sqrt(
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`p-2 rounded-lg transition-all ${
              tool === 'pen' ? 'bg-purple-500 text-white' : 'bg-gray-700 hover:bg-gray-600'
            }`}
            title="Pen"
          >
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-all ${
              tool === 'eraser' ? 'bg-purple-500 text-white' : 'bg-gray-700 hover:bg-gray-600'
            }`}
            title="Eraser"
          >
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-all ${
              tool === 'rectangle' ? 'bg-purple-500 text-white' : 'bg-gray-700 hover:bg-gray-600'
            }`}
            title="Rectangle"
          >
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-all ${
              tool === 'circle' ? 'bg-purple-500 text-white' : 'bg-gray-700 hover:bg-gray-600'
            }`}
            title="Circle"
          >
            ...
            {colors.map((c) => (
              <button
                ...
                className={`w-8 h-8 rounded-lg transition-all ${
                  color === c ? 'ring-2 ring-purple-500 ring-offset-2 ring-offset-[#252525]' : ''
                }`}
