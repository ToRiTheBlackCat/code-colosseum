...
import { Heart, Sparkles, TrendingUp, Gift, AlertCircle, Star, Gem } from 'lucide-react';
...
export const petTemplates: PetTemplate[] = [
  {
    type: 'cat',
    name: 'Code Cat',
    description: 'A curious cat that loves debugging. Purrs when you solve problems.',
    price: 500,
    currency: 'gems',
    rarity: 'Common',
    emoji: '🐱',
  ...
  },
  {
    type: 'dog',
    name: 'Algorithm Pup',
    description: 'Loyal companion that celebrates your victories. Very energetic!',
    price: 500,
    currency: 'gems',
    rarity: 'Common',
    emoji: '🐶',
  ...
  },
  {
    type: 'dragon',
    name: 'Code Dragon',
    description: 'Legendary dragon that breathes fire when you beat hard problems.',
    price: 2000,
    currency: 'gems',
    rarity: 'Legendary',
    emoji: '🐉',
  ...
  },
  {
    type: 'unicorn',
    name: 'Logic Unicorn',
    description: 'Magical unicorn that sprinkles optimization dust on your code.',
    price: 1500,
    currency: 'gems',
    rarity: 'Epic',
    emoji: '🦄',
...
export function PetSystem({ 
  ...
  const handleAdoptPet = (template: PetTemplate) => {
    const hasEnoughCurrency = template.currency === 'gems' 
  ...
  const canFeed = currentPet && dailyCheckInScore > 0 && currentPet.hunger < 100;
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {petTemplates.map((template) => {
              const canAfford = template.currency === 'gems'
                ? userGems >= template.price
                : userCoins >= template.price;
                      ...
                      <div className="flex items-center gap-2">
                        {template.currency === 'gems' ? (
                          <>
                            ...
                            <span className="text-pink-400">{template.price}</span>
                          </>
                        ) : (
                          <>
                            <Star className="w-5 h-5 text-yellow-400" />
                            <span className="text-yellow-400">{template.price}</span>
                          </>
                        )}
                      </div>
        ...
        <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 border border-purple-500/20 rounded-xl p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-gray-400">
                {petTemplates.find(p => p.type === currentPet.type)?.description}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-[#121212] border border-gray-800 rounded-lg p-4">
              ...
              <div className="text-xs text-gray-500">Level {currentPet.level}</div>
            </div>
            ...
            <div className="bg-[#121212] border border-gray-800 rounded-lg p-4">
              ...
              <div className="text-xs text-gray-500">{currentPet.happiness}%</div>
            </div>
          </div>
          ...
          {currentPet.stage !== 'adult' && (
            <div className="bg-[#121212] border border-gray-800 rounded-lg p-4 mb-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-gray-400">Evolution Progress</span>
                <span className="text-sm text-purple-400">
                  {currentPet.experience} / {getStageRequirements(currentPet.stage)} XP
                </span>
              </div>
              <div className="w-full bg-gray-800 rounded-full h-3 mb-2">
                <div 
                  className="bg-gradient-to-r from-purple-500 to-pink-500 h-3 rounded-full transition-all"
                  style={{ width: `${(currentPet.experience / getStageRequirements(currentPet.stage)) * 100}%` }}
                />
              </div>
          ...
          <div className="bg-[#121212] border border-gray-800 rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                ...
                <div className="text-2xl text-green-400">{dailyCheckInScore} pts</div>
              </div>
            </div>
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-400">
            ...
            <div>
              ...
              <span>{new Date(currentPet.acquiredDate).toLocaleDateString()}</span>
            </div>
            <div>
